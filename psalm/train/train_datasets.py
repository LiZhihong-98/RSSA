...
class COCO_panoptic_dataset(Dataset):
    def __init__(self, json_path, tokenizer, data_args, is_train=True):
        ...
        if is_train:
            ...
            self.panoptic_json_path = os.path.join(
                json_path, "annotations/panoptic_train2017.json"
            )
            ...
            self.panoptic_json_path = os.path.join(
                json_path, "annotations/panoptic_val2017.json"
            )
        ...
        self.coco_id_to_cont_id = {
            coco_id: cont_id for cont_id, coco_id in enumerate(coco_class_ids)
        }
        ...
        for source in sources:
            for sentence in source:
                ...
                if DEFAULT_SEG_TOKEN in sentence["value"]:
                    sentence["value"] = (
                        sentence["value"].replace(DEFAULT_SEG_TOKEN, "").strip()
                    )
    ...
    def preprocess_llama2(self, sources, tokenizer):
        ...
        for conversation, target in zip(conversations, targets):
            ...
            for i, rou in enumerate(rounds):
                ...
                instruction_len = (
                    len(self.tokenizer_special_tokens(parts[0], tokenizer)) - 2
                )
    ...
    def preprocess_class_name(self, CLS_token="[CAT]"):
        tokenized = [
            self.tokenizer.encode(class_name, add_special_tokens=False)
            for class_name in self.coco_class_name
        ]
        tokenized_class_names = [
            tokens + [self.tokenizer.encode(CLS_token, add_special_tokens=False)[0]]
            for tokens in tokenized
        ]
        class_name_id = [
            token for sublist in tokenized_class_names for token in sublist
        ]
        class_name_id = torch.tensor(class_name_id)
        cls_indices = [
            idx for idx, sublist in enumerate(tokenized_class_names) for _ in sublist
        ]
    ...
    def __getitem__(self, idx):
        ...
        image_file = os.path.join(
            self.panoptic_image_path, os.path.splitext(data["file_name"])[0] + ".jpg"
        )
    ...
    def __getitem__(self, idx):
        ...
        for annotation in data_dict["annotations"]:
            ...
            if annotation["category_id"] in self.coco_id_to_cont_id:
                annotation["category_id"] = self.coco_id_to_cont_id[
...
class COCO_panoptic_dataset_random(COCO_panoptic_dataset):
    def preprocess_class_name(self, CLS_token="[CAT]"):
        ...
        tokenized = [
            self.tokenizer.encode(class_name, add_special_tokens=False)
            for class_name in random_class_name
        ]
        tokenized_class_names = [
            tokens + [self.tokenizer.encode(CLS_token, add_special_tokens=False)[0]]
            for tokens in tokenized
...
class RefCOCO_dataset(COCO_instance_dataset):
    ...
    def preprocess_referring_instruction(self, instruction, REFER_token="[SEG]"):
        ...
        tokenized = tokenized + [
            self.tokenizer.encode(REFER_token, add_special_tokens=False)[0]
        ]
...
class UnifyDatasetSingleDatasetForBatch(Dataset):
    ...
    def __init__(self, datasets, dataset_ratio, bs, fix_dataset_len=0):
        ...
        for _dataset in self.datasets:
            dataset_coco_id_to_cont_id = (
                _dataset.coco_id_to_cont_id
        ...
        for _dataset in self.datasets:
            dataset_coco_class_name = (
                _dataset.coco_class_name if hasattr(_dataset, "coco_class_name") else []
            )
...
class MM_Conv_Dataset(Dataset):
        ...
        for conversation, target in zip(conversations, targets):
            ...
            if conv.version == "phi":
                ...
                for i, rou in enumerate(rounds):
                    ...
                    if idx > 0:
                        round_len = (
                            len(self.tokenizer_special_tokens(rou, tokenizer)) + 2
                        )
                    else:
                        round_len = (
                            len(self.tokenizer_special_tokens(rou, tokenizer)) + 1
                        )
                    if idx > 0:
                        instruction_len = len(
                            self.tokenizer_special_tokens(parts[0], tokenizer)
                        )
                    else:
                        instruction_len = (
                            len(self.tokenizer_special_tokens(parts[0], tokenizer)) - 1
                        )
    ...
    def tokenizer_special_tokens(
        ...
        special_indexes = [
            image_token_index if token == "<image>" else seg_token_index
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "class_name_embedding_indices" in instances[0]:
            class_name_embedding_indices = [
                instance["class_name_embedding_indices"] for instance in instances
            ]
        ...
        if "refer_embedding_indices" in instances[0]:
            ...
            refer_embedding_indices = torch.nn.utils.rnn.pad_sequence(
                refer_embedding_indices, batch_first=True, padding_value=0
            )
