...
from transformers.modeling_outputs import (
    CausalLMOutputWithPast, #  因果语言模型输出类，包含历史信息
...
from psalm.model.mask_decoder.mask_criterion.pretrain_criterion import (
    PSALM_criterion,
    hungarian_matcher_PSALM,
)
...
class PSALMModel(LlavaMetaModel, PhiModel):
    ...
    def __init__(self, config: PhiConfig, mask_decoder_cfg=None):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            self.vision_tower.image_processor["panoptic"] = (
                COCOPanopticNewBaselineDatasetMapper(self.cfg)
            )
            self.vision_tower.image_processor["instance"] = (
                COCOInstanceNewBaselineDatasetMapper(self.cfg)
            )
            self.vision_tower.image_processor["semantic"] = (
                COCOSemanticNewBaselineDatasetMapper(self.cfg)
            )
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        vision_tower.image_processor["panoptic"] = COCOPanopticNewBaselineDatasetMapper(
            self.cfg
        )
        vision_tower.image_processor["instance"] = COCOInstanceNewBaselineDatasetMapper(
            self.cfg
        )
        vision_tower.image_processor["semantic"] = COCOSemanticNewBaselineDatasetMapper(
            self.cfg
        )
        ...
        print(
            f"current mm_project_type is {self.config.mm_projector_type}, the output dim is {projector_outdim}"
        )
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            self.mm_projector.load_state_dict(
                get_w(mm_projector_weights, "mm_projector"), strict=False
            )
...
class PSALM(PhiForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(
        self, config, mask_decoder_cfg=None, add_cross_attn=True, cross_attn_index=None
    ):
        ...
        self.region_projector = nn.Linear(
            config.hidden_size, mask_decoder_cfg.MODEL.MASK_FORMER.HIDDEN_DIM
        )
    ...
    def initial_mask_module(self, pretrained_path=None, model_args=None):
        ...
        self.pixel_decoder = self.pixel_decoder_init(
            cfg=self.mask_decoder_cfg, input_shape=input_shape
        )
        self.predictor = self.predictor_init(cfg=self.mask_decoder_cfg)

        self.seg_query_projector = nn.Linear(
            self.config.hidden_size, self.mask_decoder_cfg.MODEL.MASK_FORMER.HIDDEN_DIM
        )
        self.SEG_token_projector = nn.Linear(
            self.config.hidden_size, self.mask_decoder_cfg.MODEL.MASK_FORMER.HIDDEN_DIM
        )
        self.class_name_projector = nn.Linear(
            self.config.hidden_size, self.mask_decoder_cfg.MODEL.MASK_FORMER.HIDDEN_DIM
        )
        ...
        if pretrained_path is not None:
            ...
            pixel_decoder_weights = {
                k: torch.tensor(v) for k, v in pixel_decoder_weights.items()
            }
            predictor_weights = {
                k: torch.tensor(v) for k, v in predictor_weights.items()
            }

            # deal some diff keys
            change_w(pixel_decoder_weights, "adapter_1.weight", "adapter_1.0.weight")
            change_w(
                pixel_decoder_weights, "adapter_1.norm.weight", "adapter_1.1.weight"
            )
            ...
            if predictor_weights["query_embed.weight"].shape[0] == 200:
                predictor_weights["query_embed.weight"] = predictor_weights[
                    "query_embed.weight"
                ][:100, :]
            diff_pixel_msg = self.pixel_decoder.load_state_dict(
                pixel_decoder_weights, strict=False
            )
            diff_predictor_msg = self.predictor.load_state_dict(
                predictor_weights, strict=False
            )
    ...
    def mask_decoder_training_init(self, cfg):
        ...
        weight_dict = {
            ...
            "loss_class_name_class": class_weight,
            "loss_mask": mask_weight,
            "loss_dice": dice_weight,
            "loss_region_class": class_weight,
        }
        ...
        self.sem_seg_postprocess_before_inference = (
            self.instance_on or self.panoptic_on or self.referring_on or self.region_on
        )

    def get_region_embedding(self, hidden_states, region_embedding_masks):
        region_embedding_list = []
        for sample_hidden_satates, sample_region_embedding_masks in zip(
            hidden_states, region_embedding_masks
        ):
            sample_region_embedding = sample_hidden_satates[
                sample_region_embedding_masks.bool()
            ]
    ...
    def region_inference(self, region_cls, mask_pred):
        ...
        result.scores = (
            scores * mask_scores_per_image[None, ...].repeat(scores.shape[0], 1)
        ).transpose(1, 0)
        return result
    ...
    def output_shape(self):
        ...
        for name in out_features:
            backbone_feature_shape[name] = Dict(
                {
                    "channel": out_feature_channels[name],
    ...
    def get_class_name_embedding(self, hidden_states, cls_token_indices):
        ...
        for current_hidden_state, current_token_indice in zip(
            hidden_states, cls_token_indices
        ):
            ...
            current_pool_class_name_embedding = [
                self.class_name_pooling(class_name.transpose(-2, -1)).transpose(-2, -1)
                for class_name in current_class_name_embedding_list
            ]
            class_name_embedding_list.append(
                torch.cat(current_pool_class_name_embedding, dim=0)
            )
    ...
    def embed_class_ids(self, class_name_ids, cls_indices):
        ...
        embedded_class_name = [
            self.get_model().embed_tokens(id) for id in class_name_ids
        ]
    ...
    def concat_image_seg_cls_embeds(
        ...
        if class_name_embedding_indices is not None:
            assert len(cls_token_indices) == len(
                class_embed
            ), "the number of <cls> tokens and class_embed needs to be same"
        if region_feature_list is not None:
            assert len(region_feature_list) == len(
                region_token_indices
            ), "the munber of <region> tokens and regions needs to be same"
        ...
        cur_class_name_embedding_indices = (
            [] if class_name_embedding_indices is not None else None
        )
        cur_refer_embedding_indices = (
            [] if refer_embedding_indices is not None else None
        )
        ...
        for chunk in chunks:
            ...
            if chunk_len == 1 and chunk[0] == IMAGE_TOKEN_INDEX:
                ...
                if enable_region_mask:
                    cur_new_region_embedding_mask.append(
                        torch.zeros(img_feature.shape[0])
                    )
                ...
                if enable_region_mask:
                    cur_new_region_embedding_mask.append(
                        torch.ones(region_feature.shape[0])
                    )
                ...
                if enable_region_mask:
                    cur_new_region_embedding_mask.append(
                        torch.zeros(refer_embed.shape[0])
                    )
                if class_name_embedding_indices is not None:
                    cur_class_name_embedding_indices.append(
                        torch.full(
                ...
                cur_new_input_embeds.append(
                    self.get_model().embed_tokens(input_id[:chunk_len])
                )
                cur_new_seg_query_mask.append(seg_query_mask[:chunk_len])
                if class_name_embedding_indices is not None:
                    cur_class_name_embedding_indices.append(
                        class_name_embedding_indices[:chunk_len]
                    )
                if refer_embedding_indices is not None:
                    cur_refer_embedding_indices.append(
                        refer_embedding_indices[:chunk_len]
                    )
                if label is not None:
                    cur_new_label.append(label[:chunk_len])
                if enable_region_mask:
                    cur_new_region_embedding_mask.append(
                        region_embedding_mask[:chunk_len]
                    )
        ...
        cur_new_seg_query_mask = [
            x.to(device=self.device) for x in cur_new_seg_query_mask
        ]
        cur_new_seg_query_mask = torch.cat(cur_new_seg_query_mask, dim=0)
        if class_name_embedding_indices is not None:
            cur_class_name_embedding_indices = [
                x.to(device=self.device) for x in cur_class_name_embedding_indices
            ]
            cur_class_name_embedding_indices = torch.cat(
                cur_class_name_embedding_indices, dim=0
            )
        if refer_embedding_indices is not None:
            cur_refer_embedding_indices = [
                x.to(device=self.device) for x in cur_refer_embedding_indices
            ]
            cur_refer_embedding_indices = torch.cat(cur_refer_embedding_indices, dim=0)

        if enable_region_mask:
            cur_new_region_embedding_mask = [
                x.to(device=self.device) for x in cur_new_region_embedding_mask
            ]
            cur_new_region_embedding_mask = torch.cat(
                cur_new_region_embedding_mask, dim=0
            )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        expanded_seg_query = self.seg_query.unsqueeze(0).expand(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_class_name_embedding_indices = (
                class_name_embedding_indices[batch_idx]
                if class_name_embedding_indices is not None
                else None
            )
            cur_refer_embedding_indices = (
                refer_embedding_indices[batch_idx]
                if refer_embedding_indices is not None
                else None
            )
            cur_region_feature_list = (
                region_features[batch_idx] if region_features is not None else None
            )
            cur_region_embedding_mask = (
                region_embedding_masks[batch_idx]
            ...
            cur_class_name_embedding = self.embed_class_ids(
                cur_class_name_ids, cur_cls_indices
            )
            ...
            new_seg_query_masks.append(cur_seg_query_mask)
            if class_name_embedding_indices is not None:
                new_class_name_embedding_indices.append(
                    cur_class_name_embedding_indices
                )
            if refer_embedding_indices is not None:
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            for new_seg_query_mask in new_seg_query_masks:
                new_seg_query_mask = torch.cat(
                    (
                        new_seg_query_mask,
                        torch.zeros(
                            (max_len - new_seg_query_mask.shape[0]),
                            dtype=new_seg_query_mask.dtype,
            ...
            if class_name_embedding_indices is not None:
                for new_class_name_embedding_indice in new_class_name_embedding_indices:
                    ...
                    new_class_name_embedding_indices_align.append(
                        new_class_name_embedding_indice
                    )
                new_class_name_embedding_indices = torch.stack(
                    new_class_name_embedding_indices_align, dim=0
                )
            ...
            if refer_embedding_indices is not None:
                ...
                new_refer_embedding_indices = torch.stack(
                    new_refer_embedding_indices_align, dim=0
                )
            ...
            if new_region_embedding_masks is not None:
                ...
                new_region_embedding_masks = torch.stack(
                    new_region_embedding_masks_align, dim=0
                )
            ...
            if class_name_embedding_indices is not None:
                new_class_name_embedding_indices = torch.stack(
                    new_class_name_embedding_indices, dim=0
                )
            if refer_embedding_indices is not None:
                new_refer_embedding_indices = torch.stack(
                    new_refer_embedding_indices, dim=0
                )

            if new_region_embedding_masks is not None:
                new_region_embedding_masks = torch.stack(
                    new_region_embedding_masks, dim=0
                )
    ...
    def get_SEG_embedding(self, hidden_states, refer_embedding_indices):
        ...
        for current_hidden_state, current_token_indice in zip(
            hidden_states, refer_embedding_indices
        ):
            current_refer_state = current_hidden_state[current_token_indice.bool()]
            current_pool_refer_state = self.refer_pooling(
                current_refer_state.transpose(-2, -1)
            ).transpose(-2, -1)
    ...
    def forward(
        ...
        if dataset_type is not None:
            assert all(
                item == dataset_type[0] for item in dataset_type
        ...
        if (input_ids == SEG_TOKEN_INDEX).sum() != 0:
            ...
            input_ids, attention_mask, past_key_values, inputs_embeds, labels = (
                self.mm_conv_prepare_inputs_labels_for_multimodal(
        ...
        if class_name_embedding_indices is not None:
            class_name_embedding = self.get_class_name_embedding(
                hidden_states, class_name_embedding_indices
            )
        ...
        if class_name_embedding is not None:
            class_name_embedding = torch.gather(
                class_name_embedding,
                dim=1,
                index=random_idx.unsqueeze(-1).repeat(
                    1, 1, class_name_embedding.shape[-1]
                ),
            )

        if region_embedding_masks is not None:
            region_embedding_list = self.get_region_embedding(
                hidden_states, region_embedding_masks
            )
            region_embedding_list = [
                self.region_projector(region_embedding)
                for region_embedding in region_embedding_list
            ]
        else:
        ...
        if seg_query_mask is not None:
            ...
            if refer_embedding_indices is not None:
                SEG_embedding = self.get_SEG_embedding(
                    hidden_states, refer_embedding_indices
                )
            ...
            if seg_info is not None:
                ...
                mask_loss = (
                    ...
                    + loss_dice
                    + loss_SEG_class
                    + loss_class_name_class
                    + loss_region_class
                )
                if isinstance(loss_class_name_class, float):
                    loss_class_name_class = torch.tensor(
                        loss_class_name_class, device=mask_loss.device
                    )
                if isinstance(loss_SEG_class, float):
                    loss_SEG_class = torch.tensor(
                        loss_SEG_class, device=mask_loss.device
                    )
                if isinstance(loss_region_class, float):
                    loss_region_class = torch.tensor(
                        loss_region_class, device=mask_loss.device
                    )
    ...
    def get_seg_query(self, hidden_states, seg_query_masks):
        seg_query_list = []
        for sample_hidden_state, sample_query_mask in zip(
            hidden_states, seg_query_masks
        ):
    ...
    def eval_seg(
        ...
        for (
            ...
            min_y, max_y = np.min(non_padding_indices[0]), np.max(
                non_padding_indices[0]
            )
            min_x, max_x = np.min(non_padding_indices[1]), np.max(
                non_padding_indices[1]
            )
            ...
            if self.semantic_on:
                semantic_r = retry_if_cuda_oom(self.class_name_semantic_inference)(
                    None, class_name_cls_result.float(), mask_pred_result.float()
                )
            ...
            if self.instance_on:
                instance_r = retry_if_cuda_oom(self.class_name_instance_inference)(
                    None, class_name_cls_result.float(), mask_pred_result.float()
                )
                processed_results[-1]["instances"] = instance_r
            if self.panoptic_on:
                panoptic_r = retry_if_cuda_oom(self.class_name_panoptic_inference)(
                    None, class_name_cls_result.float(), mask_pred_result.float()
                )
                processed_results[-1]["panoptic_seg"] = panoptic_r
            if self.referring_on:
                instance_r = retry_if_cuda_oom(self.SEG_instance_inference)(
                    SEG_cls_result.float(), mask_pred_result.float()
                )
                processed_results[-1]["instances"] = instance_r
            if self.region_on:
                gt = _seg_info["instances"].gt_masks
                ...
                instance_r = retry_if_cuda_oom(self.region_inference)(
                    region_cls_results.float(), mask_pred_result.float()
                )
                processed_results[-1]["instances"] = instance_r
...
class PSALMForDAVISEval(PSALM):
    def eval_seg(
        self,
        input_ids: torch.LongTensor = None, #  输入的ID张量
        attention_mask: Optional[torch.Tensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None,
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (input_ids == REGION_TOKEN_INDEX).sum() != 0 and instances is not None:
            region_masks_list = [
                instance.vp_region_masks.tensor for instance in instances
            ]
