...
class ResNetWOnorm(nn.Module):
    def __init__(self, dcn=None, out_dim=4096):
        print("using resnet without batchnorm")
        self.dcn = dcn
        self.inplanes = 256
        super(ResNetWOnorm, self).__init__()
        self.layer1 = self._make_layer(BasicBlockWOnorm, 1024, 1, stride=2, dcn=dcn)
        self.layer2 = self._make_layer(BasicBlockWOnorm, 4096, 1, stride=2, dcn=dcn)
...
class ResNetLayerNorm(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, norm_shape, stride=1, dcn=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, norm_shape, stride, downsample, dcn=dcn)
        )
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, dcn=dcn))
...
class ResNet(nn.Module):
    def __init__(self, dcn=None, out_dim=4096):
        ...
        self.layer1 = self._make_layer(BasicBlock, 1024, 1, stride=2, dcn=dcn)
        self.layer2 = self._make_layer(BasicBlock, 4096, 1, stride=2, dcn=dcn)
        self.fc = nn.Linear(4096, out_dim)
...
class ResNetSwin(nn.Module):
    def __init__(self, dcn=None, input_dim=1024, out_dim=4096):
        self.dcn = dcn
        self.inplanes = input_dim
        super(ResNetSwin, self).__init__()
        self.layer1 = self._make_layer(BasicBlock, 2048, 1, stride=2, dcn=dcn)
